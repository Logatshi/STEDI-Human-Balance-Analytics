...
accelerometer_landing = glueContext.create_dynamic_frame.from_catalog(database="stedi2", table_name="accelerometer_landing")

# Join Tables
joined_data = Join.apply(accelerometer_landing, customer_trusted, 'user', 'email')

# Write Data with Schema Evolution Enabled
S3bucket_node = glueContext.getSink(
    path="s3://stedibucket/accelerometer/trusted/",
    connection_type="s3",
