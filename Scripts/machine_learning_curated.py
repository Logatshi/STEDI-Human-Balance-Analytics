...
# Load Data
step_trainer_trusted = glueContext.create_dynamic_frame.from_catalog(database="stedi2", table_name="step_trainer_trusted")
accelerometer_trusted = glueContext.create_dynamic_frame.from_catalog(database="stedi2", table_name="accelerometer_trusted")

# Join by Timestamp
joined_data = Join.apply(step_trainer_trusted, accelerometer_trusted, 'sensorreadingtime', 'timestamp')

# Write Data with Schema Evolution Enabled
S3bucket_node = glueContext.getSink(
    path="s3://stedibucket/machine_learning/curated/",
    connection_type="s3",
...
S3bucket_node.setCatalogInfo(
    catalogDatabase="stedi2", 
